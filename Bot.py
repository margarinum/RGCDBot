...
startMessage = '''Привет! Я - бот, предназначенный для управления стендами ИСТО \n
Мой скрипт установлен на одном из серверов, но рядом с каждым приложением ИСТО развернут API, к которому я обращаюсь. 
Список серверов ты найдешь рядом, в файле "config.ini"\n
Я еще достаточно сыроват, но уже умею:\n
1. Выводить текущие установленные теги (команда *current*)\n
2. Проверять наличие новых тегов ИСТО в docker-репозитории TaskData (команда *check*)\n
3. Останавливать и запускать ИСТО (команды *stopApp* и *startApp*)\n
4. Запрашивать состояние ИСТО (команда *AppStatus*)\n
5. Запрашивать состояние автообновления (команда *AutoDeployStatus*)\n
6. Устанавливать состояние автообновления (команда *setAutodeployStatus*)\n
7. Устанавливать период проверки новыз тегов для автообновления (команда *setAutodeployStatus*)\n
8. Просто обновить стенд, если есть новые теги (команда *update*)\n
9. Запросить информацию о контейнерах (команда *containers* и выбрать контейнер)\n
10. Скачать логи контейнера (команда *containers*, выбрать контейнер и в открывшемся меню выбрать команду *download*)\n
11. (Экспериментальный функционал) Запускать минимальные автотесты системы(команда *autoTests*). После нажатия команды 
ты увидишь доступные автотесты. 
При нажатии на любой из пунктов запустится соответствующий тест на сервере и я верну тебе результат его выполнения 
и даже смогу прислать тебе подробный отчет о тестировании.\n
Ты и сам(сама) можешь прислать мне сценарий нового или дополненного автотеста, сделанный по шаблону gherkin. 
Я могу запускать файлы тестов с несколькими фичами и сценариями.  
Посмотри сценарий любого теста по команде *help* у каждого пунта автотестов. 
Файл сценария, который ты можешь мне отправить должен быть с расширением .feature, иначе я его не приму (прости)

Несколько советов по работе со мной:
1. Не торопись при работе со мной - некоторые запросы выполянются не очень быстро
2. В каких-то случаях я не смогу работать, т.к API, к которому я обращаюсь может не работать.
В этом случае я сообщу тебе об этом - при нажатии на "Серверы" будет выведена клавиатура, 
и рядом с каждой позицией будет значки \U00002705 или \U0000274C
3. Настанет момент, когда я перестану работать, 
т.к пользователь из под которого я работаю будет удален (dmitriy.litvinov)

Начнем? Нажми на кнопку "Серверы"
'''
